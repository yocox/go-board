...
export class GoBoard extends LitElement {
  ...
  override render() {
    return html`
      <canvas class=board id="go-board" @click=${this._onClick} @mousemove=${this._onMove}></canvas>
    `;
  }
  ...
  private _drawBranches(canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D) {
